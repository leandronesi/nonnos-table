...
test("mobile practice pauses, survives reload, uses Stockfish and retries a lost save response", async ({ page }) => {
  ...
  await expect(page.getByLabel("La tua mossa (notazione SAN)")).toBeHidden();
  await page.locator('[data-square="c1"]').click();
  await page.locator('[data-square="e3"]').click();
  await expect(page.locator('[data-square="e3"] [data-piece="wB"]')).toBeVisible();
  await page.clock.runFor(2000);
  ...
  const replay = page.locator('.move-playback');
  const played = await replay.getAttribute('data-position');
  await page.getByRole('button', { name: 'Mossa precedente' }).click();
  const initial = await replay.getAttribute('data-position');
  expect(initial).not.toBe(played);
  await page.getByRole('button', { name: 'Mossa successiva' }).click();
  await expect(replay).toHaveAttribute('data-position', played!);
  await page.getByRole('button', { name: 'Mossa successiva' }).click();
  await expect(replay).not.toHaveAttribute('data-position', played!);
  await page.getByRole('button', { name: 'Alternativa del motore' }).click();
  await expect(replay).toHaveAttribute('data-position', initial!);
  await replay.focus();
  await page.keyboard.press('ArrowRight');
  await expect(replay).not.toHaveAttribute('data-position', initial!);
