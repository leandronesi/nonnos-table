/**
 * OnboardingWaiting — "Il Primo Incontro".
 *
 * Una scena cinematografica: la stanza si fa sera, la lampada calda si accende,
 * Nonno parla piano. Le fasi reali dell'orchestratore guidano la narrazione.
 * Il progresso tecnico resta sullo sfondo, discreto.
 *
 ...
 *
 * Al ready: NON naviga subito. Carica coach_brief.json e mostra il primo colpo.
...
// Diverso dal CoachBrief statico legacy (types.ts): qui i campi sono
// voice_message / one_line_diagnosis, non headline / diagnosis_narrative.
interface CoachLlmBrief {
...
export function OnboardingWaiting() {
  ...
  // Stato "ready": coach_brief caricato, in attesa di CTA
  ...
  // CoachBrief = brief caricato

  // Chiave per il fade della voce Nonno (cambia con la fase)
  ...
  useEffect(() => {
    ...
    slideTimerRef.current = setTimeout(() => {
      // Cross-fade: prima nascondo, poi cambio slide, poi mostro
    ...
    runOnboardingOrchestrator({
      ...
      .then(async () => {
        ...
        if (freshRow?.onboarding_state === "ready") {
          ...
          if (!cancelled) {
            // Carica il coach_brief per il primo colpo
            try {
